...
setInterval(calculateSessionTimeout, 1000)
setInterval(closeModal, timeout)
</script>

<template>
  <Teleport to="body">
    <div v-if="open" class="modal">
      <div class="mb-6 h-1 w-full bg-transparent">
        <div
          class="fixed bottom-1 h-1 bg-{{cookiecutter.theme_color}}-500 animate-pulse"
          :style="{
            width: sessionTimeoutProgress,
            transition: `width ${sessionTimeoutProgress}ms ease-in-out`
          }"
        ></div>
      </div>
      <div class="w-full">
        <div class="flex flex-col p-5 rounded-lg shadow-2xl">
          <div class="flex flex-col items-center text-center">
            <div class="inline-block p-4 bg-{{cookiecutter.theme_color}}-50 rounded-full">
              <svg
                class="w-12 h-12 fill-current text-{{cookiecutter.theme_color}}-500"
